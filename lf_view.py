...
inputFolder = "C:/Users/Jo/Downloads/rectified/rectified/"
...
def gaussianPyramid(image):
    return list(skimage.transform.pyramid_gaussian(image, max_layer=gausslayer))#gausslayer -> max_layer
...
def alignByPyramids(referencePatch, py_target, point):
    ...
    bestDX, bestDY = 0, 0
    for ref, target, level_number in reversed(list(zip(referencePatch, py_target, range(len_py_ref)))):
        bestDX *= 2
        bestDY *= 2
